...
from src.datasets.simulated_loader import SimulatedDataset
...
def save_samples(folder_path, model, **kwargs):
    ...
    filepath = os.path.join(folder_path, f'kernel_samples')
    # Save locally
...
def main(args):
    ...
    task = 'regression'
    data_uci = SimulatedDataset(dataset=dataset_name, k=-1, normalize=True, pca_latents=params['pca_latents'], seed=0)
    ...
    for iter in range(n_sampling_iters):
        ...
        if (len(model.optimization_params_names) > 0) and (iter > 150):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="offdiag1-d5-1")
